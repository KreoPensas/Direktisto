void moreScreens(byte deviceNum) {
  ...
  if ( screen > 0 ) {
    ...
    if (screen == 27) {
      tft.drawRect(80, 0, 79, 39, YELLOW);
      ...
      recordFlipper();
      if (sequencerFlag == true) {
        tft.setCursor(14, 13);
        tft.setTextColor(RED);
        tft.print("ON");
      }
      if (sequencerFlag == false) {
        tft.setCursor(14, 13);
    ...
    if (screen == 28) {
      tft.drawRect(160, 0, 79, 39, GREEN);
      tft.setTextColor(GREEN);
      tft.setCursor(166, 13);
      tft.print("Harmony");
      if (harmonyFlag == true) {
        tft.setCursor(14, 13);
        tft.setTextColor(RED);
...
void harmonyArrayClear() {
  tft.fillRect(100, 215, 220, 20, BLACK);

}

void harmonyArrayPrint() {
  tft.setCursor(100, 215);
  tft.setTextColor(GREEN);
  tft.print(harmDevice);
  tft.print(" ");
  for (int i = 0 ; i < 16; i++) {
    tft.print(harmEnable[harmDevice][i]);
  }
}

void sequencerArrayPrint() {
  tft.setCursor(100, 215);
  tft.setTextColor(YELLOW);
  tft.print(seqDevice);
  tft.print(" ");
  for (int i = 0 ; i < 16; i++) {
    tft.print(seqEnable[seqDevice][i]);
  }
}

void recordFlipper() {
  if (recordFlag == false) {
    tft.drawRect(240, 0, 79, 39, DIMMED);
    tft.setTextColor(DIMMED);
    tft.setCursor(252, 13);
    tft.print("Record");
    tft.fillRect(240, 42, 80, 23, BLACK);
  }
  if (recordFlag == true) {
    tft.drawRect(240, 0, 79, 39, RED);
    tft.setTextColor(RED);
    tft.setCursor(252, 13);
    tft.print("Record");
    ...
    tft.setFont(Arial_10);
    tft.print("Device ");
    tft.print(recDevice + 1);
    tft.setCursor(240, 54);
    tft.print("Channel ");
    tft.print(recChannel);
    tft.setFont(Arial_12);
  }
}
...
void cpsRelabel() {
  ...
  tft.setCursor(0, 95);
  tft.setTextColor(WHITE);
  if (regiloFlag == true && regiloPort == screen - 11) {
    tft.print("Regilo");
  }
  if (pedlFlag == true && pedlPort == screen - 11) {
    tft.print("PEDL");
  }
  if (kpseqFlag == true && kpseqPort == screen - 11) {
    tft.print("K*P Seq");
  }
  colorBars(40, 160);
  printLabels = true;
