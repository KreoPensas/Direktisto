...
void catcher(byte type, byte data1, byte data2, byte channel, const uint8_t *sysexarray, byte cable)
{
  if (type == 0x80 || type == 0x90) {
    if (harmonyFlag == true && harmEnable[cable][channel-1] == true) {
      data1 = (data1 + harmSemitones);
      if ( data1 > 127) {
        data1 = 127;
        return;
      }
      ...
      sendToComputer(type, data1, data2, channel, sysexarray, cable);
    }
  }
}
